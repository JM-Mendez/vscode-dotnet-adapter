...
const uriEquals = (a: vscode.Uri, b: vscode.Uri) =>
  Object.entries(a).every(
    ([key, value]) =>
      key.startsWith("_") || value === b[key as keyof vscode.Uri]
  );

const hasGrouping = (
  patterns: SearchPatterns
): patterns is GroupedSearchPatterns =>
  typeof patterns === "object" && !Array.isArray(patterns);

const removeNodeFromParent = (
  parent: DerivitecTestSuiteInfo["parent"],
  term: string | vscode.Uri,
  prop: keyof (DerivitecTestSuiteInfo | DerivitecTestInfo) = "id"
) => {
  if (!parent) return;
  const id = parent.children.findIndex(
    (suite) =>
      (typeof suite[prop] === "string" &&
        typeof term === "string" &&
        suite[prop] === term) ||
      (suite[prop] instanceof vscode.Uri &&
        term instanceof vscode.Uri &&
        uriEquals(suite[prop] as vscode.Uri, term))
  );
  if (id === 0) {
...
const getTestFile = (file: vscode.Uri) =>
  file.with({ path: `${file.path}.txt` });

const isFileNotFound = (err: unknown) => {
  const errObj = normaliseError(err);
  return (
    ("name" in errObj && errObj.name.startsWith("EntryNotFound")) ||
    errObj.message.indexOf("non-existing file") > -1
  );
};
...
export class TestDiscovery {
  ...
  constructor(
    ...
    private readonly nodeMap: Map<
      string,
      DerivitecSuiteContext | DerivitecTestContext
    >,
  ...
  public async Load(): Promise<DerivitecTestSuiteInfo> {
    ...
    if (this.loadErrors.size > 0) {
      this.output.update(
        `Encountered errors in ${this.loadErrors.size} files during loading.`
      );
      const errArr = Array.from(this.loadErrors.entries());
      if (errArr.some(([file, err]) => err === DISCOVERY_ERROR.VSTEST_STDERR)) {
        this.output.update(
          `Some of these errors were encountered by vstest. See ".NET Core Test Output" pane for details.`
        );
      }
      const emptyErr = errArr.filter(
        ([file, err]) => err === DISCOVERY_ERROR.SYMBOL_FILE_EMPTY
      );
    ...
    if (this.SuitesInfo.children.length == 0) {
      const errorMsg =
        "No tests found, check the SearchPattern in the extension settings.";
  ...
  private async LoadFiles(searchpatterns: string[]): Promise<vscode.Uri[]> {
    ...
    await Promise.all(
      searchpatterns.map(async (pattern) => {
        const findGlob = new vscode.RelativePattern(
  ...
  private async SetTestSuiteInfo(file: vscode.Uri): Promise<void> {
    ...
    try {
      ...
      if (isFileNotFound(err)) {
        ...
        this.log.error(
          `Unable to check for a cache file for ${testListFileStr}; Encountered: ${err}`
        );
    ...
    this.Loadingtest = new Command("dotnet", args, {
  ...
  private async AddtoSuite(file: vscode.Uri) {
    ...
    for (const line of lines) {
      ...
      let classContext = this.nodeMap.get(classId) as
  ...
  private GetSuiteGroup(suite: DerivitecTestSuiteInfo) {
    ...
    if (hasGrouping(patterns) && this.SuiteGroups) {
      const name = Object.keys(patterns).find((groupName) =>
        isMatch(filePath.fsPath, patterns[groupName])
      );
  ...
  private AttachSuite(suite: DerivitecTestSuiteInfo) {
    ...
    if (this.nodeMap.has(suite.id)) {
      ...
      if (context.node.parent === parent) {
        ...
        const suiteIndex = parent.children.findIndex(
          (childSuite) =>
            childSuite.sourceDll !== "root" &&
            uriEquals(childSuite.sourceDll, filePath)
        );
  ...
  private DetachSuite(file: vscode.Uri, removeSuite: boolean = false) {
    ...
    this.nodeMap.forEach((value, key) => {
      if (
        value.node.sourceDll !== "root" &&
        uriEquals(value.node.sourceDll, file)
      ) {
  ...
  private setupWatcher(searchPattern: string) {
    ...
    const watcher = vscode.workspace.createFileSystemWatcher(
      new vscode.RelativePattern(this.workspace.uri.fsPath, searchPattern)
    );
      ...
      try {
        ...
        if (this.loadErrors.size > 0 && this.loadErrors.has(uriStr)) {
          ...
          if (loadError === DISCOVERY_ERROR.VSTEST_STDERR) {
            ...
            err +=
              "The symbols file produced was empty, are there tests in this project?";
          }
          this.output.update(err);
          this.loadErrors.clear();
        }
        ...
        this.codeLens.process(this.SuitesInfo);
        this.output.update(
          `New tests added from ${uri.fsPath.replace(
            this.workspace.uri.fsPath,
            ""
          )}`,
          true
    ...
    watcher.onDidDelete(async (uri) => {
      ...
      this.output.update(
        `Removing tests from ${uri.fsPath.replace(
          this.workspace.uri.fsPath,
          ""
        )}`,
        true
  ...
  private async handleLoadError(file: vscode.Uri, err: unknown) {
    ...
    try {
      ...
      if (isFileNotFound(errObj)) {
        this.log.error(
          `Unable to delete ${testListFile.fsPath}: ${errObj.message}`
        );
      }
    }
  }
